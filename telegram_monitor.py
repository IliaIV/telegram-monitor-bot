...
def clean_address_for_mkd(address):
    ...
    Пример:
    "Москва, ТРАО, Вороновское, мкр. Центральный (пос.ЛМС), 17, 1п., 1 этаж"
    -> "Москва, ТРАО, Вороновское, мкр. Центральный (пос.ЛМС), 17"
    """
    if not address:
        return ""
    
    # Паттерн для поиска номера дома (последний номер перед подъездом/этажом)
    # Ищем: запятая, пробел, цифры (возможно с дробью или буквой), затем запятая и подъезд/этаж
    # Сохраняем всё ДО подъезда/этажа
    patterns_to_remove = [
        r',\s*\d+п\.?\s*,?\s*',      # удаляет ", 1п." или ", 1п,"
        r',\s*\d+\s*этаж\s*,?\s*',   # удаляет ", 1 этаж" или ", 2 этаж"
        r',\s*подв\.?\s*,?\s*',      # удаляет ", подв." или ", подвал"
        ...
        r',\s*корп\.?\s*\d*\s*,?\s*', # удаляет ", корп." или ", корп. 2"
        r',\s*стр\.?\s*\d*\s*,?\s*', # удаляет ", стр." или ", стр. 2"
        r',\s*лит\.?\s*[А-Я]\s*,?\s*', # удаляет ", лит. А"
        r',\s*[а-я]\.\s*,?\s*',      # удаляет ", кв." и т.п.
    ]
